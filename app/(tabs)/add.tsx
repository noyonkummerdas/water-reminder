            ...
            <ScrollView className="flex-1 px-4 pt-4" showsVerticalScrollIndicator={false}>
                    ...
                    {DRINK_TYPES.map((drink) => {
                        ...
                        return (
                            <TouchableOpacity
                                ...
                                className="w-[30%] aspect-square bg-white p-4 rounded-[32px] m-[1.5%] shadow-sm border border-slate-50 items-center justify-center"
                ...
                <View className="mt-8 mx-2 bg-white p-10 rounded-[44px] shadow-sm border border-slate-50 items-center" style={{ opacity: isGoalReached ? 0.5 : 1 }}>
                    ...
                    <View className="flex-row w-full justify-between items-center">
                        <TouchableOpacity
                            disabled={isGoalReached}
                            className="w-16 h-16 bg-[#F8FAFB] rounded-2xl items-center justify-center border border-slate-100 shadow-sm"
                            onPress={() => setAmount(Math.max(0, amount - 50))}
                        >
                            <Minus size={24} color="#1E293B" strokeWidth={2.5} />
                        </TouchableOpacity>
                        ...
                        <View className="flex-row px-4">
                            {[100, 250, 500].map(val => (
                                <TouchableOpacity
                                    ...
                                    className={`mx-1.5 px-5 py-2.5 rounded-[18px] ${amount === val ? 'bg-[#00BDD6]' : 'bg-[#F1F5F9]'}`}
                                >
                                    <Text className={`font-black text-sm ${amount === val ? 'text-white' : 'text-[#94A3B8]'}`}>{val}</Text>
                                </TouchableOpacity>
                            ))}
                        </View>
                        ...
                        >
                            <Plus size={24} color="#1E293B" strokeWidth={2.5} />
                        </TouchableOpacity>
                    </View>
                </View>
